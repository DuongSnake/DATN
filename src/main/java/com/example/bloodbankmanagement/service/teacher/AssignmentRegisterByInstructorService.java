...
public class AssignmentRegisterByInstructorService {
    private static final Logger logger = (Logger) LoggerFactory.getLogger(AssignmentRegisterByInstructorService.class);
...
//            handleCaseInsertToStudentMapInstructor(studentInfo, request.getStatusAutoMap(), request.getInstructorId(), userIdCreate, nowDate);
    ...
    public SingleResponseDto<PageAmtListResponseDto<AssignmentStudentRegisterDto.AssignmentStudentRegisterListInfo>> selectListAssignmentRegister(AssignmentRegisterDto.AssignmentRegisterSelectListOfInstructorIdInfo request){
        ...
        Page<AssignmentStudentRegisterDTO> listDataFileMetadata = assignmentRegisterRepository.findListAssignmentRegisterByInstructorId(request, pageable);
    ...
    public SingleResponseDto<PageAmtListResponseDto<AssignmentStudentRegisterDto.AssignmentStudentRegisterListInfo>> selectListAssignmentWaitingSend(AssignmentRegisterDto.AssignmentRegisterSelectListOfInstructorIdInfo request){
        SingleResponseDto objectResponse = new SingleResponseDto();
        PageAmtListResponseDto<AssignmentStudentRegisterDto.AssignmentStudentRegisterListInfo> pageAmtObject = new PageAmtListResponseDto<>();
        request.getPageRequestDto().setPageNum(PageRequestDto.reduceValuePage(request.getPageRequestDto().getPageNum()));
        Pageable pageable = new PageRequestDto().getPageable(request.getPageRequestDto());
        //Select list file upload
        Page<AssignmentStudentRegisterDTO> listDataFileMetadata = assignmentRegisterRepository.findListAssignmentWaitingsendByInstructor(request, pageable);
        pageAmtObject = AssignmentRegister.convertListObjectToDto(listDataFileMetadata.getContent(), listDataFileMetadata.getTotalElements());
        objectResponse = responseService.getSingleResponse(pageAmtObject, new String[]{responseService.getConstI18n(CommonUtil.userValue)}, CommonUtil.querySuccess);
        return objectResponse;
    }
...
//            }
    ...
    @Transactional
    public BasicResponseDto approveAssignmentStudentRegister(AssignmentRegisterDto.AssignmentRegisterDeleteInfo listFileId, String lang){
        //De do an o trang thai bao luu
        BasicResponseDto objectResponse;
        AssignmentStudentRegister objectDelete = new AssignmentStudentRegister();
        objectDelete.setIsApproved(CommonUtil.STATUS_APPROVE);
        objectDelete.setUpdateAt(LocalDate.now());
        objectDelete.setUpdateUser(CommonUtil.getUsernameByToken());
        assignmentRegisterRepository.changeStatusAssignmentRegister(objectDelete, listFileId.getListData());
        objectResponse = responseService.getSuccessResultHaveValueMessage(CommonUtil.successValue, CommonUtil.deleteSuccess);
    ...
    public User getIdByUserInfo(String userName){
        Optional<User> userInfo = userRepository.findByUsername(userName);
        if(ObjectUtils.isEmpty(userInfo.get())){
            throw new CustomException(CommonUtil.NOT_FOUND_DATA_USER, "en");
        }
    ...
    public SingleResponseDto<PageAmtListResponseDto<AssignmentStudentRegisterDto.AssignmentStudentRegisterListInfo>> findListAssignmentRegisterIsApprove(AssignmentRegisterDto.AssignmentRegisterSelectListOfInstructorIdInfo request){
        SingleResponseDto objectResponse = new SingleResponseDto();
        ...
        Pageable pageable = new PageRequestDto().getPageable(request.getPageRequestDto());
        //Select list file upload
        Page<AssignmentStudentRegisterDTO> listDataFileMetadata = assignmentRegisterRepository.findListAssignmentRegisterIsApproveByInstructorId(request, pageable);
        pageAmtObject = AssignmentRegister.convertListObjectToDto(listDataFileMetadata.getContent(), listDataFileMetadata.getTotalElements());
        objectResponse = responseService.getSingleResponse(pageAmtObject, new String[]{responseService.getConstI18n(CommonUtil.userValue)}, CommonUtil.querySuccess);
        return objectResponse;
    }
    ...
    public SingleResponseDto<List<UserDto.UserSelectListInfo>> selectListStudentMapWithInstructor(UserDto.UserSelectInfo request, String lang)  throws Exception{
        ...
        List<UserInfoDto> listDataUser = userRepository.getListUserByRoleNameMapWithInstructorId(ERole.ROLE_USER.toString(), request.getId());
        pageAmtObject = User.convertListStudentByInstructor(listDataUser);
        objectResponse = responseService.getSingleResponse(pageAmtObject, new String[]{responseService.getConstI18n(CommonUtil.userValue)}, CommonUtil.querySuccess);
        return objectResponse;
    }

    public SingleResponseDto<List<UserDto.UserSelectListInfo>> listStudentAlreadyMapInstructorNotHaveAssignemt (UserDto.UserSelectInfo request, String lang) throws Exception {
        if(ObjectUtils.isEmpty(request)){
            throw new Exception("Request data not null");
        }
        if(StringUtils.isEmpty(request.getId())){
        ...
        List<UserInfoDto> listDataUser = userRepository.getListStudentMapWithInstructorButNotRegisterAssignment(ERole.ROLE_USER.toString(), request.getId(), CommonUtil.STATUS_RESERVE, CommonUtil.STATUS_EXPIRE);
        pageAmtObject = User.convertListStudentByInstructor(listDataUser);
        objectResponse = responseService.getSingleResponse(pageAmtObject, new String[]{responseService.getConstI18n(CommonUtil.userValue)}, CommonUtil.querySuccess);
        return objectResponse;
    }
