...
public class ClassRoomServiceImpl {
    ...
    @Transactional
    public BasicResponseDto insertClassRoom(ClassRoomDto.ClassRoomInsertInfo request, String lang) {
        ...
        ClassRoom objectUpdate = new ClassRoom();
        objectUpdate.setClassName(request.getClassName());
        //Find the value major by Id
        ...
        objectUpdate.setMajor(objectMajor);
        ...
        classRoomRepository.save(objectUpdate);
        result = responseService.getSuccessResultHaveValueMessage(CommonUtil.successValue, CommonUtil.insertSuccess);
        return result;
    }

    public SingleResponseDto<PageAmtListResponseDto<ClassRoomDto.ClassRoomListInfo>> selectListClassRoom(ClassRoomDto.ClassRoomSelectListInfo request){
        SingleResponseDto objectResponse = new SingleResponseDto();
        //Set value userId
        String userId = isUserHaveRoleAdmin(request.getCreateUser());
        request.setCreateUser(userId);
        PageAmtListResponseDto<ClassRoomDto.ClassRoomListInfo> pageAmtObject = new PageAmtListResponseDto<>();
        Pageable pageable = new PageRequestDto().getPageable(request.getPageRequestDto());
        //Select list file upload
        Page<ClassRoom> listDataFileMetadata = classRoomRepository.findListClassRoom(request, pageable);
        pageAmtObject = ClassRoom.convertListObjectToDto(listDataFileMetadata.getContent());
        objectResponse = responseService.getSingleResponse(pageAmtObject, new String[]{responseService.getConstI18n(CommonUtil.userValue)}, CommonUtil.querySuccess);
        return objectResponse;
    }

    public SingleResponseDto<ClassRoomDto.ClassRoomSelectInfoResponse> selectClassRoom(ClassRoomDto.ClassRoomSelectInfo request, String lang){
        if(null == request || request.getClassId().equals("") || null == request.getClassId()){
            throw new CustomException("Not found value request param ", "en");
        }
        SingleResponseDto<ClassRoomDto.ClassRoomSelectInfoResponse> selectObject = new SingleResponseDto<>();
        ClassRoom dataFileMetadata = classRoomRepository.findByFileId(request.getClassId());
        ClassRoomDto.ClassRoomSelectInfoResponse objectResponse= new ClassRoomDto.ClassRoomSelectInfoResponse();
        if(null ==dataFileMetadata){
            objectResponse = null;
        }else{
            objectResponse = ClassRoom.convertToDto(dataFileMetadata);
        }
    ...
    @Transactional
    public BasicResponseDto updateClassRoom(ClassRoomDto.ClassRoomUpdateInfo request, String lang){
    ...
    public BasicResponseDto deleteClassRoom(ClassRoomDto.ClassRoomDeleteInfo listFileId, String lang){
        ...
        classRoomRepository.deleteClassRoom(objectDelete, listFileId.getListClassRoomId());
